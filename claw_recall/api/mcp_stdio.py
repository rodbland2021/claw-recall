...
Runs over stdio for local MCP clients. For remote access, use mcp_sse.py.

Usage:
    python3 -m claw_recall.api.mcp_stdio               # stdio mode (for MCP clients)
...
For SSE/HTTP transport (remote agents), see mcp_sse.py.
"""
...
from claw_recall.search.engine import resolve_agent
...
def search_thoughts(
    ...
    Args:
        ...
        limit: Max results
    """
    from claw_recall.search.engine import search_thoughts as _search_thoughts
...
def capture_thought(
    ...
    """
    from claw_recall.capture.thoughts import capture_thought as _capture
...
def browse_activity(
    ...
    agent = resolve_agent(agent) if agent else agent
...
def poll_sources(
    ...
    """
    from claw_recall.capture.sources import poll_gmail, poll_drive, poll_slack
...
def memory_stats() -> str:
    ...
    from claw_recall.database import get_db_stats
    from claw_recall.search.engine import cache_status
